...
class App extends Component {
  ...
  constructor(props) {
    ...
    this.state = {
      inferenceURL: 'pipeline-car-evaluation',
      inferenceResult: '',
      payloadfinal: '',
      buying: 'low',
      maintained: 'high',
  ...
  handleInference(event) {
    ...
    this.setState({
      payloadfinal: 'Payload sent to sagenaker endpoint: ' + payload
    })
    
    console.log('myInit', myInit);
    let apiName = 'sagemaker';
    this.setState({
      inferenceResult: 'Invoking sagemaker endpoint'
    })
    let path = '/items';
    ...
    API.get(apiName, path, myInit).then(response => {
      this.setState({
        inferenceResult: 'Inference result: success:' + JSON.stringify(response)
      ...
      this.setState({
        inferenceResult: 'Inference result: error:' + JSON.stringify(error.response)
      })
    });
  }
            ...
            <Form>
              ...
              <Form.Field>
                ...
                />
                   ...
                   <Segment>{this.state.payloadfinal} <br/> {this.state.inferenceResult}</Segment>
            </Container>
            </div>
    );
  }
